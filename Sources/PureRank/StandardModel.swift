...
func stdPdf(_ t: Double) -> Double {
  exp(-0.5 * pow(t, 2)) / sqrt(2 * .pi)
}

/// Calculates standard normal cumulative distribution function Φ(t) (CDF)
func stdCdf(_ t: Double) -> Double {
  0.5 * (1 + erf(t / sqrt(2)))
}

/// Calculates standard normal percent point function Φ⁻¹(p) (inverse CDF)
func stdPpf(_ p: Double) -> Double {
